...
Everything stateful lives on two Railway volumes: `mysql` (`/var/lib/mysql`, Den database) and `worker` (`/data`: projects, sessions/OpenCode state, global and per-project engine config, server.json project list, plugin/template installs, tokens, temp). Redeploys and restarts keep volumes; deleting the service or volume does not. Railway volumes are single-region with no built-in replication, so enable volume backups (Settings → Volume → Backups, or `volumeInstanceBackupScheduleUpdate` with `DAILY`/`WEEKLY`) and take a snapshot before risky changes. API keys pasted into engine config files (`opencode.json`) are on the volume in plaintext; prefer Settings → Environment for secrets.
