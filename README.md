...
- **Private networking.** den-web reaches den-api at `http://den-api.railway.internal:8788`. den-api listens on all interfaces (Node dual-stack); den-web is started with `--hostname ::`. The worker keeps `--host 0.0.0.0` because `openwork-server` derives its internal engine URL from the bind host and does not handle `::`.
...
- **Email.** Invitations need `RESEND_API_KEY` or `SMTP_*`; without them, add members by having them sign in with an allowed owner email or enable public signup temporarily.
